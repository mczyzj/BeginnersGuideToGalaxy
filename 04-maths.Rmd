...
parameters <- c(b     = 0.1,       # birth rate
                d     = 0.1,       # death rate
                beta  = 0.0002,    # transmission parameter
                gamma = 0.5,       # 1/gamma = infectious period
                a     = 0.4        # 1/a = latent period
                )
...
E0 = 0     # initialy exposed
I0 = 1     # initialy infected
R0 = 0     # initialy recovered
...
modelOutput <- deSolve::ode(y = initial, func = SEIR, times = times, parms = parameters)
head(modelOutput)
```

### Ploting SERI model
To create a plot using our favorite library `ggplot2` (which will be described with more details in chapter \@ref(Graphics)) we first need to convert our modelOutput to object of class *data frame*. Than we can use code below to plot our model.
